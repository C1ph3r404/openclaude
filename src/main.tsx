...
>>>>>>> origin/main
              ...
              // Handle resume flow - from file (internal-only), session ID, or interactive selector
              // DEBUG: Create a marker file to confirm we're in this code path
              try {
                const fs = require('fs');
                fs.writeFileSync('/tmp/openclaude-resume-marker.txt', `Resume triggered at ${new Date().toISOString()}\n`);
              } catch { }
              ...
              let processedResume: ProcessedResume | undefined = undefined;
              let maybeSessionId = validateUuid(options.resume);
              // DEBUG logging - ALWAYS log when resume is called
              try {
                const fs = require('fs');
                ...
                fs.appendFileSync(
                  ...
                  `[${new Date().toISOString()}] [BROWSERLLM] main: OPTIONS.RESUME TRIGGERED with maybeSessionId=${maybeSessionId}, options.resume=${options.resume}\n`,
                  { mode: 0o600 }
                );
              } catch (e) {
                console.error('[DEBUG] Failed to log resume start:', e);
              }
              let searchTerm: string | undefined = undefined;
              // Store full LogOption when found by custom title (for cross-worktree resume)
              let matchedLog: LogOption | null = null;
...
>>>>>>> origin/main
              ...
              if (maybeSessionId) {
                ...
                try {
                  ...
                  // DEBUG: Log if we're about to call processResumedConversation
                  try {
                    const fs = await import('fs');
                    fs.appendFileSync(
                      require('path').join(process.env.HOME || '/root', '.config', 'claude', 'debug', 'debug.log'),
                      `[${new Date().toISOString()}] [BROWSERLLM] main.tsx: About to call processResumedConversation with browserLLMConvoId=${result.browserLLMConvoId}\n`,
                      { mode: 0o600 }
                    );
                  } catch { }
                  processedResume = await processResumedConversation(result, {
